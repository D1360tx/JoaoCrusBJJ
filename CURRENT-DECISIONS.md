...
- Preserve both comparison drafts on their review branch. Superseded discovery restriction: the follow-up approved on 2026-09-24 adds `/resources/` after About in primary navigation and in visitor footers on the 23 standard indexable academy/Parent Guide pages. Add contextual discovery in Home’s Joao authority section, About’s Words Beyond the Mat, and only Joao’s Coaches profile. Keep the dedicated Resources layout and existing booking footer link intact. Exclude noindex campaign, quiz, thank-you, legal/special pages and the Practice Under Pressure funnel from this rollout.
- Release this discovery change as HTML only from the authoritative merged build. Restore only each target’s exact existing live `campaign-site.js?v=…` reference, prove normalized pre-change output equals live byte-for-byte, then prove staged differences contain only the reviewed discovery additions. Do not overwrite shared runtime, assets, APIs, hosting config, sitemap, robots, consent, or attribution. Require a limited manifest, full rollback backup, reviewed dry run, residual zero and public hash equality.
