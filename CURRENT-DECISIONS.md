...
## Castle Hill four-ad live normalization (2026-09-09 05:49:52 UTC — current)

- User explicitly authorized URL normalization and selective publication/reactivation. This supersedes every older paused-only or two-ad launch decision below.
- Campaign `120251246135250072` and ad set `120251246144560072` independently read `ACTIVE / ACTIVE`; campaign daily budget remains `1000` ($10/day). Exact parent payloads, targeting and website/Lead/pixel configuration are unchanged.
- Exactly four ads are `ACTIVE / ACTIVE`: AY07 `120251261010900072` → `1387970049521888` (was `1687998043330822`); AY08 `120251261015960072` → `1051476211136471` (was `4302676583376329`); AY09A `120251263139970072` → verified existing candidate `1571675311324124` (was `1559827305832313`); AY09B `120251263002380072` → unchanged `1567708678432946`.
- The other seven ads, including original AY09 `120251261045730072`, remain unchanged `PAUSED / PAUSED`. No campaign/ad-set pauses, targeting changes, budget changes, copy changes, media changes or funnel changes were made.
- All four active creatives have clean `https://joaocrusbjj.com/castle-hill-grand-opening/` and exact dedicated `url_tags`: `utm_source=meta&utm_medium=paid_social&utm_campaign=austin_castle_hill_launch_v1&utm_content={{ad.name}}&utm_term={{adset.name}}&utm_id={{campaign.id}}`. Recursive validation confirms exactly one of each key, raw macros, no manual fbclid and no form extension.
- All replacement creatives were independently verified before live swaps. Each changed ad was paused, swapped while explicitly PAUSED, read back PAUSED/PAUSED with full content/tracking checks, then activated and read back ACTIVE/ACTIVE. AY09B was not written. AY09A preserves exact existing video IDs `1911621773147370` and `2175548869693779`; copy, identities, labels, image hashes, CTA and placement rules match before/after.
- Meta returns additional explicitly disabled enhancement keys for AY08 and the existing AY09A candidate; every preexisting opt-out is unchanged and no enhancement is enabled. Generated creative names/post IDs and tracking-array order can differ; Page, pixel, conversion IDs and action types are preserved. A foreground execution timeout was recovered from fresh state without replaying completed swaps.
- Evidence: `assets/meta/castle-hill/youth-wave-2/comparisons/reconciliation-2026-09-08/live-four-ad-normalization.json` includes independent final readback, before state, replacement checks and serial operation evidence. Signed thumbnail URLs are omitted from public evidence; raw responses remain private. `scripts/validate_castle_live_four_ads.py` and six positive/negative tests pass. Earlier evidence files remain immutable historical snapshots. API state is confirmed; this update does not claim a new browser funnel submission or a fresh Ads Manager draft-panel inspection.

## Castle Hill E2E certification and accepted compatibility residual (2026-09-09 UTC; historical activation state)
