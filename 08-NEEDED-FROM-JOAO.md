...
| 3.3 | **CONFIRM (don't assume): is ANY email-service provider / ESP connected or in use anywhere?** Explicitly ask about **ActiveCampaign, Mailchimp, Constant Contact, ConvertKit/Kit, Klaviyo, HubSpot, Flodesk, GetResponse, MailerLite, AWeber** — or one bundled inside his website builder, ads agency, or booking tool. Get a yes/no per "do you log into any email tool to send newsletters/broadcasts?" **AND** any existing email LIST (ESP exports, Skool members, book buyers, camp parents). | We must VERIFY this, not infer it. Site audit (03) found no ESP wired into the website form — but that does NOT rule out a standalone account he adds contacts to manually, or one his agency runs. Treat "he has nothing" as unconfirmed until he says so directly. Determines whether we connect to an existing ESP or stand one up; also seed list + consent assessment | ⬜ |
...
| 6.8 | **"Helping schools incorporate BJJ" (his priority #2) — which schools?** ✅ ANSWERED (2026-06-11): **martial-arts schools** (Line B as built) + interest in **corporate speaking** (→ Line C). After-school programs = open exploration (07 §6A) | Resolved; after-school pilot question goes to Joao in Phase 2 | ✅ |
| 6.9 | 🆕 **Confirm the age-3 policy:** does he officially accept kids under 4? Any insurance/capacity/case-by-case caveats? Is he comfortable ADVERTISING "from age 3"? | He's the ONLY local school taking 3-year-olds (first-party — it's why I enrolled my son) and the site doesn't even say it. Pending confirmation, it becomes a headline landing-page claim (07 §3) | ⬜ |
