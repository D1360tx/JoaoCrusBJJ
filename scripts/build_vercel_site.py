...
GTM_HEAD_SNIPPET = rf"""<!-- Google Tag Manager -->
    ...
    if(u.searchParams.has(k)){{var raw=u.searchParams.get(k);var ids=['utm_id','campaign_id','adset_id','ad_id','gclid','fbclid','wbraid','gbraid','msclkid'];var v=ids.indexOf(k)!==-1 ? (/^[A-Za-z0-9._~-]{{1,160}}$/.test(raw||'') ? raw : '') : safeCampaignValue(raw);if(v)safe.searchParams.set(k,v);}}}});
