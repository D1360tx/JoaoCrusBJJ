...
GTM_HEAD_SNIPPET = rf"""<!-- Google Tag Manager -->
    ...
    {json.dumps(['utm_source', 'utm_medium', 'utm_campaign', 'utm_content', 'utm_term', 'utm_id', 'campaign_id', 'campaign_name', 'adset_id', 'adset_name', 'ad_id', 'ad_name', 'placement', 'cta_placement', 'site_source_name', 'gclid', 'fbclid', 'wbraid', 'gbraid', 'msclkid', 'qa', 'gtm_debug', 'gtm_auth', 'gtm_preview', 'gtm_cookies_win'])}.forEach(function(k){{
    if(u.searchParams.has(k)){{var raw=u.searchParams.get(k);var ids=['utm_id','campaign_id','adset_id','ad_id','gclid','fbclid','wbraid','gbraid','msclkid'];var clickIds=['gclid','fbclid','wbraid','gbraid','msclkid'];var lim=clickIds.indexOf(k)!==-1?512:160;var v=ids.indexOf(k)!==-1 ? ((raw||'').length<=lim&&/^[A-Za-z0-9._~-]+$/.test(raw||'') ? raw : '') : safeCampaignValue(raw);if(v)safe.searchParams.set(k,v);}}}});
