...
const url = process.env.QA_URL || 'http://127.0.0.1:8874/resources/';
assert.ok(['127.0.0.1', 'localhost'].includes(new URL(url).hostname), 'QA must use localhost');
const output = process.env.QA_OUTPUT || '/home/d1360/releases/resources-20260924/visual';
  ...
  try {
    for (const width of [390, 768, 1280, 1440, 1920]) {
      ...
      await context.route('**/*', route => {
        if (route.request().url().startsWith('https://api.country.is/')) return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({country:'DE'})});
