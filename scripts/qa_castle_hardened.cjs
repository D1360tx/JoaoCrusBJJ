...
(async()=>{
 ...
 const evidence=[];
 for(const width of [390,1440]) for(const consent of ['granted','denied']) for(const outcome of ['accepted','rejected','wrong-event','missing-note','wrong-request']) {
  ...
  await context.route('**/*',async route=>{
   ...
   if(req.method()!=='GET'){
    ...
    return route.fulfill({status:outcome==='rejected'?502:200,json:{accepted:true,contact_accepted:true,opportunity_accepted:true,note_accepted:outcome!=='missing-note',request_id:outcome==='wrong-request'?'qa_wrong_request':payload.request_id,meta_event_id:outcome==='wrong-event'?'lead_wrong_event_id':'lead_'+payload.request_id}});
  ...
  await page.waitForFunction(expected=>window.joaoConsentRegion && window.joaoConsentState?.analytics_storage===expected,consent);
  ...
  evidence.push({width,consent,outcome,posts,ga4:count,meta:state.meta.length});await context.close();
 }
