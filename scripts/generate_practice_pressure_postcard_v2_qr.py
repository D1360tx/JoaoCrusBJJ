...
TARGET = (
    ...
    "&utm_content=postcard_v2"
)
OUT = Path(__file__).resolve().parents[1] / "site" / "assets" / "practice-under-pressure-postcard-v2"
OUT.mkdir(parents=True, exist_ok=True)

qr = qrcode.QRCode(version=None, error_correction=ERROR_CORRECT_H, box_size=16, border=4)
qr.add_data(TARGET)
qr.make(fit=True)
png_path = OUT / "practice-under-pressure-postcard-v2-qr.png"
svg_path = OUT / "practice-under-pressure-postcard-v2-qr.svg"
with png_path.open("wb") as stream:
    qr.make_image(fill_color="#101010", back_color="#ffffff").save(stream)
