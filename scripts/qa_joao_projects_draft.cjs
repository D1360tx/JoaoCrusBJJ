...
const url = process.env.QA_URL || 'http://127.0.0.1:8769/campaign/joao-projects-draft.html';
assert.ok(['127.0.0.1', 'localhost'].includes(new URL(url).hostname), 'QA must use localhost');
const output = process.env.QA_OUTPUT || '/tmp/joao-projects-draft-qa';
  ...
  try {
    for (const width of [390, 768, 1280, 1440, 1920]) {
      ...
      const geometry = await page.evaluate(() => {
        ...
        const clipped = [...document.querySelectorAll('h1,h2,.jp-button,.jp-browser,.jp-blueprint-word')].filter(el => el.scrollWidth > el.clientWidth + 1).map(el => el.textContent.trim());
        return {
          ...
          buttons: [...document.querySelectorAll('a.jp-button')].map(el => ({ foreground: getComputedStyle(el).color, background: getComputedStyle(el).backgroundColor, height: el.getBoundingClientRect().height })),
      ...
      await page.locator('.jp-footer').scrollIntoViewIfNeeded();
      await page.screenshot({ path: path.join(output, `draft-${width}-bottom.png`) });
      await page.locator('.jp-closing a').click();
      assert.equal(new URL(page.url()).hash, '#projects');
      assert.ok(Math.abs(await page.locator('#projects').evaluate(el => el.getBoundingClientRect().top)) <= 2);
      await page.locator('.jp-hero .jp-button').click();
      assert.equal(new URL(page.url()).hash, '#projects');
      await page.goto(url); await page.keyboard.press('Tab');
      assert.equal(await page.evaluate(() => document.activeElement.className), 'jp-skip');
      ...
      const disabled = page.locator('.jp-disabled');
      assert.ok(await disabled.isDisabled());
      await disabled.evaluate(el => el.click());
      assert.equal(context.pages().length, 1);
      assert.equal(external.length, 4, 'Only the four intercepted CTA navigations');
      assert.deepEqual(errors, []); assert.deepEqual(failed, []); assert.deepEqual(responses, []);
      results.push({ width, geometry, axeViolations: 0, consoleErrors: errors, failedRequests: failed, httpErrors: responses, interceptedCTAs: external, screenshots: 8, interactions: 'skip, hero, return, 4 keyboard new-tab CTAs, disabled Blueprint passed' });
      fs.writeFileSync(path.join(output, 'browser-results.json'), JSON.stringify(results, null, 2));
      console.log(`PASS ${width}px: geometry, headings, images, fonts, contrast, axe, links, keyboard, disabled state, 8 screenshots`);
      await context.close();
    }
    ...
    assert.equal(await page.locator('article a').count(), 4);
    assert.ok(await page.locator('.jp-disabled').isDisabled());
    console.log('PASS no-JavaScript fallback: 5 projects, 4 links, Blueprint disabled');
    await context.close();
