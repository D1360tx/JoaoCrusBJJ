...
- Visitors in the EEA, United Kingdom, and Switzerland must opt in before `analytics_storage` is granted. A failed, missing, malformed, or unrecognized country lookup follows the same strict path. An explicit saved choice overrides the regional default.
- The analytics choice is mirrored across first-party browser preference stores, including the strictly necessary `joao_consent_v1` cookie (up to one year), so a withdrawal remains bootstrap-readable when one storage API is unavailable.
