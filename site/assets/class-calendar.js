(function () {
  ...
  var CLASSES = [
    {
      day: 0,
      time: "5:00–5:45 PM",
      name: "Little Champions (Ages 3–7)",
      program: "little",
      groups: ["little"],
      location: "ds",
    },
    {
      day: 0,
      time: "5:50–6:35 PM",
      name: "Junior Warriors (Ages 8–12)",
      program: "junior",
      groups: ["youth"],
      location: "ds",
    },
    ...
    },
    {
      day: 1,
      time: "10:00–10:45 AM",
      name: "Homeschool Kids (Ages 4–12)",
      program: "homeschool",
      groups: ["youth"],
      location: "ds",
    },
  ...
  function validProgram(value) {
    return PROGRAMS.some(function (item) {
  ...
  function validLocation(value) {
    return value === "ds" || value === "austin" ? value : "all";
  }
  function programLabel(value) {
    var match = PROGRAMS.find(function (item) {
      return item.id === value;
    });
    return match ? match.label : "All classes";
  }
  function matchesProgram(item, program) {
    return (
      program === "all" ||
      (program === "kids" &&
        (item.groups.indexOf("little") !== -1 ||
          item.groups.indexOf("youth") !== -1)) ||
      item.groups.indexOf(program) !== -1
    );
  }
  ...
  function initCalendar(root, index) {
    ...
    root.innerHTML =
      ...
      "</div>" +
      '<p class="jc-calendar-summary" id="' +
      summaryId +
      '" aria-live="polite"></p>' +
    ...
    function render() {
      var filtered = CLASSES.filter(function (item) {
        return (
          matchesProgram(item, selectedProgram) &&
          (selectedLocation === "all" || item.location === selectedLocation)
        );
      });
      var byDay = DAYS.map(function (_, day) {
      ...
      DAYS.forEach(function (day) {
        html +=
          '<div class="jc-calendar-day" role="columnheader">' + day + "</div>";
      });
      for (var row = 0; row < rows; row++) {
        byDay.forEach(function (items) {
          ...
          if (!item) {
            html +=
              '<div class="jc-calendar-blank" role="cell">' +
              (row === 0 && !items.length ? "—" : "") +
              "</div>";
            return;
          }
          var featured =
            pageProgram !== "all" && matchesProgram(item, pageProgram);
          html +=
            '<div class="jc-calendar-slot' +
            (featured ? " is-featured" : "") +
            '" role="cell">' +
            '<span class="jc-calendar-time">' +
            item.time +
            "</span>" +
            '<span class="jc-calendar-name">' +
            item.name +
            "</span>" +
            '<span class="jc-calendar-location">' +
    ...
    root.addEventListener("click", function (event) {
      ...
      if (programButton) {
        ...
        root.querySelectorAll("[data-program]").forEach(function (button) {
          button.setAttribute("aria-pressed", String(button === programButton));
        });
        render();
      }
      if (locationButton) {
    ...
    root
      .querySelector(".jc-calendar-print")
      .addEventListener("click", function () {
        document.body.classList.add("jc-printing");
        window.print();
      });
    window.addEventListener("afterprint", function () {
      document.body.classList.remove("jc-printing");
    });
    render();
  }
